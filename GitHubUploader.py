...
#            print('指定したユーザ {0} はDBに存在しません。GitHubUserRegister.pyで登録してください。')
        ...
        loop.run_until_complete(self.__UpdateAllUserContributions(path_dir_db, username=args.username))
#        loop.run_until_complete(self.__UpdateAllUserContributions(path_dir_db))
        loop.close()
...
#        user = web.service.github.api.v3.CurrentUser.CurrentUser(self.__db, args.username)
    ...
    async def __UpdateAllUserContributions(self, path_dir_db, username=None):
